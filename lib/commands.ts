...
// The two commands that flip meme mode. The hosts need to recognise them by
// name to know whether the rule applies to the turn they are about to run.
export enum MemeCommandName {
  On = 'aiwaysmeme-on',
  Off = 'aiwaysmeme-off',
}

export interface MemeCommand {
  name: string;
  description: string;
  template: string;
}
...
// load the command from their own package root, so the paths that follow it are
...
const COMMAND_PREFIX = 'aiwaysmeme-';
const MARKDOWN = '.md';
const ARGUMENTS = '$ARGUMENTS';
const COMMAND_DIRECTORY = join(packageRoot, 'commands');

const parse = (name: string, content: string): MemeCommand => {
...
export const readCommands = (): MemeCommand[] =>
